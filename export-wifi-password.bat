@echo off
@REM Original copyright provided bellow. 
@REM Modifications made in order to store results in file instead of printing in consle.

@REM Modified by: https://github.com/ckaridis
...
set wifiFileName="wifi-passwords.txt"
...
:main
    ...
    :main-next-profile
        for /f "tokens=1* delims=," %%a in ("%r%") do (
            ...
            if "!key!" NEQ "" (
                echo WiFi: [%%a] Password: [!key!] >> %wifiFileName%
            )
            set r=%%b
        )
